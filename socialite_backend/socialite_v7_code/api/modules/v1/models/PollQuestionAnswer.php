...
class PollQuestionAnswer extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['poll_id','poll_question_id','question_option_id','status'], 'required'],
            
            [['poll_id','id','created_at','poll_question_id','question_option_id','status','user_id'], 'integer'],
            // [['title'], 'string'],
            
            [['poll_question_id','poll_id'], 'required','on'=>['create','update']],
    ...
    {
        return $this->hasMany(PollQuestionOption::className(), ['question_id' => 'id'])->andOnCondition(['poll_qustion_options.status' => PollQuestionOption::STATUS_ACTIVE])->all();
