...
class PodcastCategory extends \yii\db\ActiveRecord
{
    ...
    {
        return ['subCategory','podcastList'];
    }
    ...
    public function getParent(){
        ...
        return $this->hasOne(PodcastCategory::className(), ['id' => 'parent_id']);

    }

    public function getSubCategory(){

        return $this->hasMany(PodcastCategory::className(), ['parent_id' => 'id'])->from(['subCategory' => PodcastCategory::tableName()])->select(['id','name','parent_id']);
    ...
    {
        return $this->hasMany(Podcast::className(), ['category_id' => 'id'])->andOnCondition(['podcast.status' => Podcast::STATUS_ACTIVE])->limit(10);
